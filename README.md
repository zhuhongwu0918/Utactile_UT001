...
- [windows平台下使用触觉传感器(RS485)](#windows平台下使用触觉传感器rs485)
  - [1. 下载并安装MinGW](#1-下载并安装mingw)
  - [2. 安装gcc后确认是否安装成功](#2-安装gcc后确认是否安装成功)
  - [3. 编译并运行工程，获取原始16进制数据](#3-编译并运行工程获取原始16进制数据)
  - [4. 输出传感器数据](#4-输出传感器数据)
- [windos平台下使用触觉传感器(CAN)](#windos平台下使用触觉传感器can)
- [UTactile触觉传感器通信协议](#utactile触觉传感器通信协议)
  - [协议说明](#协议说明)
  - [数据流格式说明](#数据流格式说明)
    - [数据流](#数据流)
    - [头码](#头码)
  ...
  - [数据包详细说明](#数据包详细说明)
    - [上位机发送的数据包](#上位机发送的数据包)
      - [0x80 修改驱动卡内的基准数据](#0x80-修改驱动卡内的基准数据)
      - [0x81 读取当前数据采集卡内部基准数据](#0x81-读取当前数据采集卡内部基准数据)
      - [0x82设置驱动卡上传的传感器数据类型](#0x82设置驱动卡上传的传感器数据类型)
      - [0x83设置传感器数据上报频率](#0x83设置传感器数据上报频率)
      - [0x84获取传感器数据上报频率](#0x84获取传感器数据上报频率)
  - [驱动卡发送的数据包](#驱动卡发送的数据包)
    - [0x00 传感器阵列基准后数据](#0x00-传感器阵列基准后数据)
    - [0x01 传感器阵列源数据数据](#0x01-传感器阵列源数据数据)
...
## 驱动卡发送的数据包
### 0x00 传感器阵列基准后数据
高位在前，低位在后传输，例如当Byte[3~4]:01 02是，第1个传感器x轴真实数据为0x102。
|字节|含义|备注|
|---|---|---|
|Byte[3]|第1个传感器x轴数据||
|Byte[5~6]|第1个传感器y轴数据||
